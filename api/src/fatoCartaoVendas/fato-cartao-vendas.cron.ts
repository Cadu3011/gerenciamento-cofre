...
@Injectable()
export class FatoCartaoVendasCron {
  @Inject()
  private readonly fatoService: FatoCartaoVendasService;
  ...
  async execute(context: JobExecutionContext, options: RunJobQueryDto = {}) {
    ...
    await context.startDateProgress('FatoCartaoVendas', start, end);
    ...
    try {
      ...
      await context.updateDateProgress('FatoCartaoVendas', end);
      return resultado;
    } catch (error) {
