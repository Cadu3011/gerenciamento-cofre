...
export class TrierApiClient {
  ...
  async getVendas(
    ...
    const res = await fetch(
      `${process.env.API_TRIER_URL}/sgfpod1/rest/integracao/venda/obter-todos-v1?primeiroRegistro=0&quantidadeRegistros=999&dataEmissaoInicial=${date}&dataEmissaoFinal=${date}`,
      { headers: { Authorization: `Bearer ${tokenLocalTrier}` } },
    );
  ...
  async getCancelamentos(
    ...
    const res = await fetch(
      `${process.env.API_TRIER_URL}/sgfpod1/rest/integracao/venda/cancelamento/obter-todos-v1?primeiroRegistro=0&quantidadeRegistros=999&dataEmissaoInicial=${date}&dataEmissaoFinal=${date}`,
      { headers: { Authorization: `Bearer ${tokenLocalTrier}` } },
    );
