...
import SumMovements from "@/components/sumMovements";
              ...
              >
                ...
                <div className="mb-3 p-3 bg-blue-100 rounded-lg">
                  ...
                  <ul className="flex flex-wrap gap-4 text-blue-800 font-semibold">
                    {["DESPESA", "DEPOSITO", "SANGRIA", "OUTRAS_ENTRADAS"].map(
                      (type) => (
                        <li key={type}>
                          {type}:{" "}
                          <SumMovements
                            type={type}
                            filialId={String(f.filial.id)}
                            token="" // passe o token se necessário
                          />
                        </li>
                      )
                    )}
                  ...
                  {s.movements.length === 0 ? (
                    ...
                    <ul className="space-y-2">
                      {s.movements.map((m, i) => (
                        ...
                        >
                          <span className="text-gray-700">{m.descrition}</span>
                          <span
                ...
                <div>
                  ...
                  {s.balanceFisic.length === 0 ? (
                    ...
                    s.balanceFisic.map((b) => (
                      <div
                        key={b.id}
                        className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-white p-3 rounded-lg shadow-sm border border-gray-100"
                      >
                        ...
                        <div className="text-sm text-gray-600">
                          <span className="font-medium">5:</span> R$ {b.value_5}
                        </div>
                        <div className="text-sm text-gray-600">
                          <span className="font-medium">2:</span> R$ {b.value_2}
                        </div>
                        <div className="text-sm text-gray-600">
