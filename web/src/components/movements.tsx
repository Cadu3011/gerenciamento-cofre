...
export default function ExibirMovimentos({ type, filialId, token }: Props) {
  ...
  const [loadingId, setLoadingId] = useState<number | null>(null); // controla item em loading
  const { refresh, updatedAt } = useCofreFisic();

  const deleteMovements = async (id: number) => {
    setLoadingId(id); // inicia loading nesse item
    try {
      await deleteMoves(id);
      refresh(); // vai atualizar a lista
    } catch (err) {
      console.error("Erro ao deletar:", err);
    } finally {
      setLoadingId(null); // reseta quando acabar
    }
  };
      ...
      <ul className="overflow-y-auto max-h-40 divide-y divide-gray-200">
        {movements.map((move: any, index: number) => (
          ...
          >
            ...
            <strong className="w-1/3 text-center text-black">
              {move.value}
            </strong>
            ...
            <button
              ...
              disabled={loadingId === move.id} // desativa só esse botão
